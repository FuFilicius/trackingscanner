...
class ScanMaster:
    def __init__(self, options: dict[str, object], worker_count: int):
    ...
    def queue_url(self, url: str, job_id: int | None = None) -> int:
        ...
        self.task_queue.put((current_job_id, normalize_target_url(url)))
        self._queued_jobs += 1
    ...
    def get_result(self, timeout: float = 0.5) -> tuple[int, ScanResultPayload] | None:
        if not self._started:
