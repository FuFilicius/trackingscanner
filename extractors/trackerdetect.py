...
class TrackerExtractor(Extractor):
	...
	def extract_information(self) -> None:
		...
		trackers_summary: TrackersSummary = {
		...
		self.result["trackers"] = trackers_summary
	...
	def _tag_tracker_requests(self) -> tuple[set[str], set[str], int, dict[str, int]]:
		...
		match_cache: dict[tuple[str, str | None], bool] = {}
		...
		for request in self.data.request_log.values():
			...
			document_hostname = urlparse(document_url).hostname if document_url else None


			request_url = request.url[:150]

			cache_key = (request_url, document_hostname)

			cached_result = match_cache.get(cache_key)
			if cached_result is None:
				is_tracker = self._matches_tracker_rule(request_url, document_hostname)
				match_cache[cache_key] = is_tracker
			else:
				is_tracker = cached_result
	...
	def _matches_tracker_rule(self, request_url: str, document_hostname: str | None) -> bool:
		...
		if document_hostname:
			options["domain"] = document_hostname
	...
	def _load_rules(self) -> None:
		...
		self.rules = AdblockRules(raw_rules, use_re2=False)
