from extractors.base import Extractor
from scanner_tools.results import CountByKey, RequestsSummary
from utils import get_corresponding_response, parse_domain


class RequestsExtractor(Extractor):
    def extract_information(self) -> None:
        summary: RequestsSummary = {
            "total": 0,
            "with_response": 0,
            "set_cookie": 0,
            "methods": {},
            "resource_types": {},
            "status_classes": {},
            "counts_by_fqdn": {},
        }

        for request in self.data.request_log.values():
            summary["total"] += 1
            response = get_corresponding_response(request.request_id, self.data)

            fqdn = parse_domain(request.url).fqdn or (request.parsed_url.hostname or "unknown")
            self._increase_counter(summary["counts_by_fqdn"], fqdn)


            self._increase_counter(summary["methods"], request.method)

            resource_type = response.resource_type if response else request.resource_type
            self._increase_counter(summary["resource_types"], resource_type)

            if response is not None:
                summary["with_response"] += 1
                status_class = f"{response.status // 100}xx"
                self._increase_counter(summary["status_classes"], status_class)

            if self._get_sets_cookie(response):
                summary["set_cookie"] += 1

        self.result["requests"] = summary

    @staticmethod
    def _get_sets_cookie(response: object) -> bool:
        if response is None:
            return False
        headers_lower = getattr(response, "headers_lower", None)
        if not isinstance(headers_lower, dict):
            return False
        return "set-cookie" in headers_lower

    @staticmethod
    def _increase_counter(counter_dict: CountByKey, key: str) -> None:
        normalized = key or "unknown"
        counter_dict[normalized] = counter_dict.get(normalized, 0) + 1
