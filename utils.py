...
class CookieEntry:
    ...
    @classmethod
    def from_playwright_cookie(cls, cookie: dict[str, object]) -> "CookieEntry":
        return cls(
...
class ScanData:
    ...
    local_storage: dict[str, object] = field(default_factory=dict)
    local_storage_by_origin: list[dict[str, object]] = field(default_factory=list)
    event_tasks: set[object] = field(default_factory=set)
    on_request_handler: object | None = None
    on_response_handler: object | None = None
    on_request_finished_handler: object | None = None
    on_request_failed_handler: object | None = None
...
def local_storage_for_page_url(
    page_url: str, local_storage_by_origin: list[dict[str, object]]
) -> dict[str, object]:
    if not page_url:
        return {}
...
TARGET_URL_SCHEME_RE = re.compile(r"^[a-zA-Z][a-zA-Z0-9+.-]*://")


def normalize_target_url(url: str, default_scheme: str = "https") -> str:
    normalized_input = url.strip()
    if not normalized_input:
        return normalized_input
    if TARGET_URL_SCHEME_RE.match(normalized_input):
        return normalized_input
    if normalized_input.startswith("//"):
        return f"{default_scheme}:{normalized_input}"
    return f"{default_scheme}://{normalized_input}"
