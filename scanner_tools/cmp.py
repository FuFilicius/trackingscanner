...
_STRIP_CHARS = " \t\r\n✓›!.,;:|/\\()[]{}<>\"'`~_-"
# Small read timeouts are a common source of flakiness on slow pages.
_READ_TIMEOUT_MS = 300
# Avoid getting stuck querying frames that haven't reached a usable execution context yet.
_FRAME_LOAD_TIMEOUT_MS = 500
...
class CMPInteractor:
    def __init__(self, options: dict[str, Any] | None = None):
        ...
        self.user_interaction_enabled = bool(options.get("cmp_user_interaction", True))
        self.user_interaction_delay_ms = int(options.get("cmp_user_interaction_delay_ms", 3000))
        self.pre_click_wait_ms = int(options.get("cmp_pre_click_wait_ms", 500))
        ...
        self.accept_text_pattern = self._build_accept_text_pattern(tuple(self.accept_words.values()))
    ...
    def try_accept(self, page: Page) -> CMPInteractionResult:
        ...
        for frame in self._ordered_frames(page):
    ...
    def _simulate_user_interaction(self, page: Page) -> None:
        """Deterministic interaction to help trigger banners without introducing randomness."""
        try:
            page.mouse.move(50, 50)
            page.mouse.move(150, 150, steps=8)
            page.mouse.wheel(0, 120)
        except (PlaywrightTimeoutError, PlaywrightError):
            return

        if self.user_interaction_delay_ms > 0:
            try:
                page.wait_for_timeout(self.user_interaction_delay_ms)
            except (PlaywrightTimeoutError, PlaywrightError):
                return

    def _ordered_frames(self, page: Page) -> list[Frame]:
        frames = list(page.frames)
        frames.sort(key=self._frame_sort_key)
        return frames

    @staticmethod
    def _frame_sort_key(frame: Frame) -> tuple[int, str, str]:
        # Main frame first, then stable URL (without query/fragment), then name.
        try:
        ...
        return (0 if is_main else 1, stable_url, name)

    def _should_skip_frame(self, frame: Frame) -> bool:
        try:
    ...
    def _find_and_click_exact_match(
        ...
        try:
            locator.first.wait_for(state="attached", timeout=300)
            count = locator.count()
    ...
    def _build_accept_text_pattern(words: tuple[str, ...]) -> re.Pattern[str]:
        escaped_words = [re.escape(word).replace("/", "\\/") for word in words if word]
