...
5. Results are returned as a dictionary keyed by scanned URL and saved to `test-results/scan_results_<timestamp>.json`.
...
URL inputs can be full URLs or bare domains. If no scheme is provided (for example `google.com`), the scanner automatically uses `https://`.
...
- `--screenshots` - capture before/after consent phase screenshots (stored locally)
- `--screenshots-dir <path>` - directory to write screenshots when `--screenshots` is enabled
...
example_result = results["https://example.com"]
```
...
- `cmp_user_interaction` (default: `True`) - simulate minimal user interaction before trying to accept CMPs.
- `cmp_user_interaction_delay_ms` (default: `3000`) - wait after simulated interaction before scanning for accept buttons.
- `cmp_pre_click_wait_ms` (default: `500`) - wait after initial load before trying consent interaction.
...
- `screenshots_enabled` (default: `False`) - capture before/after consent phase screenshots (stored locally, not in results).
- `screenshot_dir` (required when screenshots enabled) - local directory to write screenshot images.
...
- extractor summaries like `requests`, `failed_requests` (summary only), `cookies`
...
The session recored list (`resources/session_recorders.json`) is from [Blacklight](https://github.com/the-markup/blacklight-collector)
The tracker list (`resources/easyprivacy.txt`) is from [Easylist](https://easylist.to/)
